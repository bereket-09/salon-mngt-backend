const { Service, Branch } = require('../models/index')
const { Sequelize } = require('sequelize')

const { Op } = Sequelize;

// Create a new service
exports.createService = async function createService(req, res) {
  const {
...
// List all services
exports.listServices = async function listServices(req, res) {
  try {
...
// Update a service
exports.updateService = async function updateService(req, res) {
  const { id } = req.params;
  const {
...
// Delete a service
exports.deleteService = async function deleteService(req, res) {
  const { id } = req.params;
  try {
    const service = await Service.findByPk(id);
    if (!service) return res.status(404).json({ error: 'Service not found' });
